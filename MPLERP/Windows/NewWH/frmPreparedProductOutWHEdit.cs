...
namespace MLTERP
{
    public partial class frmPreparedProductOutWHEdit : frmModuleBaseWHEdit
    {
        #region 全局变量
        /// <summary>
        /// 加载单据类型
        ...
        public frmPreparedProductOutWHEdit()
        {
            InitializeComponent();
        }
        ...
        /// 加载计划单
        /// </summary>
        private void WHLoadTowelProductionPlan()
        {
        ...
        private void WHLoadTowelProductionPlan(string p_Str)
        {
            ...
            for (int i = 0; i < itembuyid.Length; i++)
            {
                ...
                if (dt.Rows.Count == 1)
                {
                    string CPItemCode = SysConvert.ToString(dt.Rows[0]["ItemCode"]);
                    string sqlA = " select * from Data_ItemDts where MainID in (select ID from Data_Item where ItemCode = " + SysString.ToDBString(CPItemCode) + ")";
                    DataTable dtA = SysUtils.Fill(sqlA);
                    if (dtA.Rows.Count > 0)//成品没有半成品
                    {
                        for (int j = 0; j < dtA.Rows.Count; j++)
                        {
                            gridView1.SetRowCellValue(setRowID, "ItemCode", SysConvert.ToString(dtA.Rows[j]["DtsItemCode"]));
                            gridView1.SetRowCellValue(setRowID, "ItemName", SysConvert.ToString(dtA.Rows[j]["DtsItemName"]));
                            gridView1.SetRowCellValue(setRowID, "ItemStd", SysConvert.ToString(dtA.Rows[j]["DtsItemStd"]));
                            gridView1.SetRowCellValue(setRowID, "ItemModel", SysConvert.ToString(dtA.Rows[j]["DtsItemModel"]));
                            ...
                            gridView1.SetRowCellValue(setRowID, "LLQty", SysConvert.ToDecimal(dt.Rows[0]["LLQty"]));
                            gridView1.SetRowCellValue(setRowID, "LLUnit", SysConvert.ToString(dt.Rows[0]["LLUnit"]));
