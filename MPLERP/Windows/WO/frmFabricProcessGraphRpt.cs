...
namespace MLTERP
{
    ...
    /// 染整加工图形报表
    /// </summary>
    public partial class frmFabricProcessGraphRpt : frmAPBaseUIRpt
    {
        public frmFabricProcessGraphRpt()
        {
            InitializeComponent();
        }
        ...
        private DataTable GetDataSource(int year)
        {
            ...
            sql += "   FROM UV1_WO_FabricProcessDts WHERE SubmitFlag=1 AND MakeDate ";
            ...
            sql += " AND ProcessTypeID=" + SysString.ToDBString(this.FormListAID); 
        ...
        private void CreateGraph(ZedGraphControl zgc, DataTable dtS)
        {
            ...
            myPane.Title.Text = drpCheckYear.Text + "年 月加工量柱状图  (单位：吨)";
            myPane.XAxis.Title.Text = "月份";
            myPane.YAxis.Title.Text = "加工量";
            ...
            BarItem myCurve = myPane.AddBar("加工量", list, Color.Blue);//使用哪一种图类
        ...
        private void CreateGraphCur(ZedGraphControl zgc, DataTable[] dtS,int[] p_YI)
        {
            ...
            //myPane.xAxisList.Clear();
            // Set the titles and axis labels
            myPane.Title.Text = (SysConvert.ToInt32(drpCheckYear.Text) - SysConvert.ToInt32(drpCheckYearNum.Text)).ToString() + "--" + drpCheckYear.Text + "年 月加工量对比图  (单位：吨)";
            myPane.XAxis.Title.Text = "月份";
            myPane.YAxis.Title.Text = "加工量";
        ...
        private DataTable GetDataSourceVendorAmount(int year)
        {
            ...
            sql += "(SELECT Sum(Amount) Amount,DyeFactorty VendorID";
            sql += "  ";
            sql += "   FROM UV1_WO_FabricProcessDts WHERE SubmitFlag=1 AND MakeDate  ";
        ...
        private void CreateGraphSOTypeSource(ZedGraphControl zgc, DataTable dtS)
        {
            ...
            //myPane.xAxisList.Clear();
            // Set the titles and axis labels
            myPane.Title.Text = drpCheckYear.Text + "年 加工量饼图  (单位：吨)";
            //myPane.XAxis.Title.Text = "月份";
            myPane.YAxis.Title.Text = "加工量";
        ...
        private DataTable GetDataSourceVendorQty(int year)
        {
            ...
            sql += " Right OUTER JOIN ";
            sql += "(SELECT Sum(Qty) SQty,DyeFactorty VendorID";
            sql += "  ";
            sql += "   FROM UV1_WO_FabricProcessDts WHERE SubmitFlag=1 AND MakeDate  ";
            if (!chkIndate.Checked)
            {
        ...
        private DataTable GetDataSourceVendorSouce(int year)
        {
            ...
            sql += " Right OUTER JOIN ";
            sql += "(SELECT Sum(Qty) SQty,Sum(Amount) as Amount,DyeFactorty VendorID";
            sql += "  ";
            sql += "   FROM UV1_WO_FabricProcessDts WHERE SubmitFlag=1 AND MakeDate  ";
            //sql += "   BETWEEN " + SysString.ToDBString(year.ToString() + "-01-01") + " AND " + SysString.ToDBString(year.ToString() + "-12-31 23:59:59");
            if (!chkIndate.Checked)
        ...
        private void frmYarnGraphRpt_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Common.BindVendor(drpQVendorID, new int[] { (int)EnumVendorType.织厂, (int)EnumVendorType.染厂, (int)EnumVendorType.加工户, (int)EnumVendorType.其他加工厂}, true);
