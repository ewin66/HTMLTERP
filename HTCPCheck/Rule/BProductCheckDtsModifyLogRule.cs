...
{
	/// <summary>
	/// 目的：WO_BProductCheckDtsModifyLog实体业务规则类
	/// 作者:朱小涛
	/// 创建日期:2013/8/28
	/// </summary>
	public class BProductCheckDtsModifyLogRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public BProductCheckDtsModifyLogRule()
		{
		}
        ...
        //    BProductCheckDtsModifyLog entity = (BProductCheckDtsModifyLog)p_BE;
        //    bool ret = false;
        //    string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, BProductCheckDtsModifyLog.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
