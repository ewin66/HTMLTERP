...
namespace HttSoft.MLTERP.DataCtl
{
    ...
    public class FormNoControlRule
    {
        ...
        /// <returns></returns>
        private string RGetFormNo(string p_ClsA, string p_ClsB, int p_SubTypeID, IDBTransAccess sqlTrans)
        {
            string outstr = string.Empty;
            FNORelRule frule = new FNORelRule();

            int fcontrolID = frule.RGetFormNoControlID(p_ClsA, p_ClsB, p_SubTypeID, sqlTrans);

            if (fcontrolID != 0)
            {
                outstr = this.RGetFormNo(fcontrolID, sqlTrans);
            }
        ...
        /// </summary>
        /// <param name="p_FormNoID">单据ID</param>
        public string RGetWHFormNo(int p_FormListID, string p_WHID, IDBTransAccess sqlTrans)
        {
            try
            {
                string sql = "SELECT FormNoControlID FROM Enum_FormList WHERE ID=" + SysString.ToDBString(p_FormListID);
        ...
        /// </summary>
        /// <param name="p_FormNoID">单据ID</param>
        public string RGetWHFormNo(int p_SubType, IDBTransAccess sqlTrans)
        {
            try
            {
                string sql = "SELECT FormNoControlID FROM Enum_FormList WHERE ID=" + SysString.ToDBString(p_SubType);
                DataTable dt = sqlTrans.Fill(sql);
                if (dt.Rows.Count != 0)
                {
                    return this.RGetFormNo(SysConvert.ToInt32(dt.Rows[0]["FormNoControlID"].ToString()), 0,  sqlTrans);
                }
                return "";
            }
            catch (BaseException)
        ...
        /// <param name="p_FormNoID">单据ID</param>
        /// <param name="p_Num">第几个单据号码0,1,2,</param>
        public string RGetFormNo2(int p_FormNoID, string p_VendorID)
        {
            try
            {
                IDBTransAccess sqlTrans = TransSysUtils.GetDBTransAccess();
                try
                {
                    ...
                    string outstr = this.RGetFormNo2(p_FormNoID, p_VendorID, sqlTrans);
        ...
        /// 获得新生产单号
        /// </summary>
        /// <param name="p_JarNo"></param>
        public string RGetFormNo2(int p_ID, string p_VendorID, IDBTransAccess sqlTrans)
        {

            string outstr = string.Empty;
            if (p_VendorID != string.Empty)
            {
                string sql = string.Empty;
                string str = string.Empty;
                str = "SR" + SysConvert.ToString(DateTime.Now.ToString("yy")) + p_VendorID;
                sql = "SELECT MAX(FormNoIndex)  FROM Sale_ProductionNotice Where FormNo LIKE " + SysString.ToDBString(str + "%");
                DataTable dt = sqlTrans.Fill(sql);
                int jarIndex = 1;
                if (dt.Rows.Count > 0)
                {
                    string tempStr = dt.Rows[0][0].ToString();
                    jarIndex = SysConvert.ToInt32(tempStr) + 1;
                }

                outstr = str + SysString.IntToStr(jarIndex, 2);
