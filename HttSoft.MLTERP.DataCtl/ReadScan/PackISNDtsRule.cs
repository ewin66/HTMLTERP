...
{
	/// <summary>
	/// 目的：WO_PackISNDts实体业务规则类
	/// 作者:Johnny
	/// 创建日期:2012/5/26
	/// </summary>
	public class PackISNDtsRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public PackISNDtsRule()
		{
		}
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            PackISNDts entity = (PackISNDts)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, PackISNDts.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM WO_PackISNDts WHERE 1=1";
                sql += p_condition;
