...
{
	/// <summary>
	/// 目的：Att_ProductCheck实体业务规则类
	/// 作者:刘德苏
	/// 创建日期:2012/5/8
	/// </summary>
	public class ProductCheckRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public ProductCheckRule()
		{
		}
		...
		{
			ProductCheck entity=(ProductCheck)p_BE;
		}	
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            ProductCheck entity = (ProductCheck)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, ProductCheck.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Att_ProductCheck WHERE 1=1";
                sql += p_condition;
        ...
        public DataTable RShowDts(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Attn_ProductCheckDts WHERE 1=1";
                sql += p_condition;
                return SysUtils.Fill(sql);
            }
            catch (BaseException)
        ...
        public void RAdd(BaseEntity p_BE, BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                ProductCheckDtsRule ruledts = new ProductCheckDtsRule();
                ruledts.RSave((ProductCheck)p_BE, p_BE2, sqlTrans);//保存从表
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				ProductCheck entity=(ProductCheck)p_BE;
                string sql = "SELECT FormNo FROM Att_ProductCheck WHERE FormNo="+SysString.ToDBString(entity.FormNo);
                DataTable dt = sqlTrans.Fill(sql);
                if (dt.Rows.Count > 0)
                {
                    throw new BaseException("单号已存在，请重新生成");
                }
				ProductCheckCtl control=new ProductCheckCtl(sqlTrans);
				entity.ID=(int)EntityIDTable.GetID((long)SysEntity.Att_ProductCheck,sqlTrans);
				control.AddNew(entity);
                FormNoControlRule rulest = new FormNoControlRule();
                rulest.RAddSort((int)FormNoControlEnum.成品检验单号,sqlTrans);
			}
			catch(BaseException)
			{
				throw;
		...
		public void RDelete(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
                ...
                string sql = "DELETE FROM Att_ProductCheckDts WHERE MainID=" + entity.ID.ToString();
