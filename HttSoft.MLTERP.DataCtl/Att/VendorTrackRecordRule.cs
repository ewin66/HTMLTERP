...
{
	/// <summary>
	/// 目的：Att_VendorTrackRecord实体业务规则类
	/// 作者:陈加海
	/// 创建日期:2014/7/16
	/// </summary>
	public class VendorTrackRecordRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public VendorTrackRecordRule()
		{
		}
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Att_VendorTrackRecord WHERE 1=1";
                sql += p_condition;
		...
		public void RUpdate(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				this.CheckCorrect(p_BE);
				VendorTrackRecord entity=(VendorTrackRecord)p_BE;
                string sql = "SELECT ID FROM Att_VendorTrackRecord WHERE ID<>" + entity.ID + " AND FormNo=" + SysString.ToDBString(entity.FormNo);
                DataTable dt = sqlTrans.Fill(sql);
                if (dt.Rows.Count > 0)
                {
                    throw new BaseException("单号已存在，请检查后重新输入");
                }
