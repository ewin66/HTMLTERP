...
{
	/// <summary>
	/// 目的：Sale_QuotedPrice实体业务规则类
	/// 作者:刘德苏
	/// 创建日期:2012/5/3
	/// </summary>
	public class QuotedPriceRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public QuotedPriceRule()
		{
		}
        ...
        public void RAdd(BaseEntity p_BE, BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                QuotedPriceDtsRule ruledts = new QuotedPriceDtsRule();
                ruledts.RSave((QuotedPrice)p_BE, p_BE2, sqlTrans);//保存从表

                FormNoControlRule fnrule = new FormNoControlRule();
                fnrule.RAddSort("Sale_QuotedPrice", "FormNo", 0, sqlTrans);
            }
            catch (BaseException)
            {
                throw;
