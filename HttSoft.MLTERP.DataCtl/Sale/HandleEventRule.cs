...
{
	/// <summary>
	/// 目的：Sale_HandleEvent实体业务规则类
	/// 作者:tanghao
	/// 创建日期:2015/5/19
	/// </summary>
	public class HandleEventRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public HandleEventRule()
		{
		}
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            HandleEvent entity = (HandleEvent)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, HandleEvent.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Sale_HandleEvent WHERE 1=1";
                sql += p_condition;
