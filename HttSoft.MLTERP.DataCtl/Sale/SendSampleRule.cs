...
{
	/// <summary>
	/// 目的：Sale_SendSample实体业务规则类
	/// 作者:章文强
	/// 创建日期:2014/12/11
	/// </summary>
	public class SendSampleRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public SendSampleRule()
		{
		}
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            SendSample entity = (SendSample)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, SendSample.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Sale_SendSample WHERE 1=1";
                sql += p_condition;
