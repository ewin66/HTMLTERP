	...
	public class DaoTongRule
	{
        ...
        public void RAdd(BaseEntity p_BE, BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                ruledts.RSave((DaoTong)p_BE, p_BE2, sqlTrans);//保存从表
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
                ...
                {
                    throw new Exception("倒筒单号已经存在，请重新生成");
                }

                //entity.ID = (int)EntityIDTable.GetID((long)SysEntity.Sale_DaoTong, sqlTrans);
                control.AddNew(entity);
                //sql = " UPDATE Enum_FormNoControl SET CurSort=CurSort+1 WHERE ID=" + (int)FormNoControlEnum.倒筒单号;//更新CurSort
