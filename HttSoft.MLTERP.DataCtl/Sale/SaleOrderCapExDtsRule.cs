...
{
	/// <summary>
	/// 目的：Sale_SaleOrderCapExDts实体业务规则类
	/// 作者:章文强
	/// 创建日期:2012/7/30
	/// </summary>
	public class SaleOrderCapExDtsRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public SaleOrderCapExDtsRule()
		{
		}
		...
		{
			SaleOrderCapExDts entity=(SaleOrderCapExDts)p_BE;
		}	
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            SaleOrderCapExDts entity = (SaleOrderCapExDts)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, SaleOrderCapExDts.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM Sale_SaleOrderCapExDts WHERE 1=1";
                sql += p_condition;
        ...
        {
            SaleOrderCapExDts entityDts = new SaleOrderCapExDts();
            ...
            sql = "SELECT ID FROM Sale_SaleOrder WHERE FormNo=" + SysString.ToDBString(htentity.HTNo);
            ...
            sql = "SELECT ISNULL(MAX(Seq),0)+1 AS MSeq FROM Sale_SaleOrderCapExDts WHERE MainID=" + SysString.ToDBString(entityDts.MainID);
        ...
        public void RHTCancel(RecPayHTDts htentity, IDBTransAccess sqlTrans)
        {
            SaleOrderCapExDts entityDts = new SaleOrderCapExDts();
            string sql = string.Empty;
            sql = "SELECT ID FROM Sale_SaleOrderCapExDts WHERE ExRemark=" + SysString.ToDBString(htentity.ID);
            DataTable dt = sqlTrans.Fill(sql);
            if (dt.Rows.Count != 0)
            {
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				SaleOrderCapExDts entity=(SaleOrderCapExDts)p_BE;				
				SaleOrderCapExDtsCtl control=new SaleOrderCapExDtsCtl(sqlTrans);
				entity.ID=(int)EntityIDTable.GetID((long)SysEntity.Sale_SaleOrderCapExDts,sqlTrans);
