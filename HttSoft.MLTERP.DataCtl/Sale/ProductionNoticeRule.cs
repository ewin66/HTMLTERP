...
{
	/// <summary>
	/// 目的：Sale_ProductionNotice实体业务规则类
	/// 作者:章文强
	/// 创建日期:2014/10/15
	/// </summary>
	public class ProductionNoticeRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public ProductionNoticeRule()
		{
		}
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            ProductionNotice entity = (ProductionNotice)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, ProductionNotice.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM Sale_ProductionNotice WHERE 1=1";
                sql += p_condition;
        ...
        public DataTable RShowDts(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Sale_ProductionNoticeDts WHERE 1=1";
                sql += p_condition;
                return SysUtils.Fill(sql);
            }
            catch (BaseException)
