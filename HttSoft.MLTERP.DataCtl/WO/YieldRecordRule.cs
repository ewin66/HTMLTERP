...
{
	/// <summary>
	/// 目的：WO_YieldRecord实体业务规则类
	/// 作者:周富春
	/// 创建日期:2014/9/25
	/// </summary>
	public class YieldRecordRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public YieldRecordRule()
		{
		}
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            YieldRecord entity = (YieldRecord)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, YieldRecord.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM WO_YieldRecord WHERE 1=1";
                sql += p_condition;
        ...
        public DataTable RShowDts(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_WO_YieldRecordDts WHERE 1=1";
                sql += p_condition;
                return SysUtils.Fill(sql);
            }
            catch (BaseException)
        ...
        public void RAdd(BaseEntity p_BE, BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                YieldRecordDtsRule ruledts = new YieldRecordDtsRule();
                ruledts.RSave((YieldRecord)p_BE, p_BE2, sqlTrans);//保存从表
		...
		public void RDelete(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
                ...
                string sql = "DELETE FROM WO_YieldRecordDts WHERE MainID=" + entity.ID.ToString();
