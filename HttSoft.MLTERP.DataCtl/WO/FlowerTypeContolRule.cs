...
{
	/// <summary>
	/// 目的：Data_FlowerTypeContol实体业务规则类
	/// 作者:丛远晶
	/// 创建日期:2012/5/24
	/// </summary>
	public class FlowerTypeContolRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public FlowerTypeContolRule()
		{
		}
		...
		{
			FlowerTypeContol entity=(FlowerTypeContol)p_BE;
		}	
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            FlowerTypeContol entity = (FlowerTypeContol)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, FlowerTypeContol.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM Data_FlowerTypeContol WHERE 1=1";
                sql += p_condition;
                return SysUtils.Fill(sql);
            }
            catch (BaseException)
        ...
        public void RAdd(BaseEntity p_BE, BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                FlowerTypeContolDtsRule ruledts = new FlowerTypeContolDtsRule();
                ruledts.RSave((FlowerTypeContol)p_BE, p_BE2, sqlTrans);//保存从表
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				FlowerTypeContol entity=(FlowerTypeContol)p_BE;				
				FlowerTypeContolCtl control=new FlowerTypeContolCtl(sqlTrans);
				entity.ID=(int)EntityIDTable.GetID((long)SysEntity.Data_FlowerTypeContol,sqlTrans);
