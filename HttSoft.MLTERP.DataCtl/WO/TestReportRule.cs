...
{
	/// <summary>
	/// 目的：WO_TestReport实体业务规则类
	/// 作者:tanghao
	/// 创建日期:2015/5/25
	/// </summary>
	public class TestReportRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public TestReportRule()
		{
		}
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            TestReport entity = (TestReport)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, TestReport.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM WO_TestReport WHERE 1=1";
                sql += p_condition;
        ...
        public void RAdd(BaseEntity p_BE, BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                TestReportDtsRule ruledts = new TestReportDtsRule();
                ruledts.RSave((TestReport)p_BE, p_BE2, sqlTrans);//保存从表
