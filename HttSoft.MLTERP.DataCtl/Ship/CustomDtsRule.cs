...
{
	/// <summary>
	/// 目的：Ship_CustomDts实体业务规则类
	/// 作者:潘杰俊
	/// 创建日期:2009-4-28
	/// </summary>
	public class CustomDtsRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public CustomDtsRule()
		{
		}
		...
		{
			CustomDts entity=(CustomDts)p_BE;
		}	
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM Ship_CustomDts WHERE 1=1";
                sql += p_condition;
        ...
        public void RSave(Custom p_Entity, BaseEntity[] p_BE, IDBTransAccess sqlTrans)
        {
            try
            {
                string sql = "DELETE FROM Ship_CustomDts WHERE MainID=" + p_Entity.ID.ToString();
                sqlTrans.ExecuteNonQuery(sql);//删除原单据明细数据
                for (int i = 0; i < p_BE.Length; i++)
                {
                    CustomDts entitydts = (CustomDts)p_BE[i];
                    sql = "SELECT ISNULL(MAX(Seq),0)+1 As MSEQ FROM Ship_CustomDts WHERE MainID=" + p_Entity.ID.ToString();
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				CustomDts entity=(CustomDts)p_BE;				
				CustomDtsCtl control=new CustomDtsCtl(sqlTrans);
