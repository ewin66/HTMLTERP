...
{
	/// <summary>
	/// 目的：WH_IOFormDtsISN实体业务规则类
	/// 作者:周富春
	/// 创建日期:2015/4/13
	/// </summary>
	public class IOFormDtsISNRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public IOFormDtsISNRule()
		{
		}
		...
		{
			IOFormDtsISN entity=(IOFormDtsISN)p_BE;
		}	
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            IOFormDtsISN entity = (IOFormDtsISN)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, IOFormDtsISN.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				IOFormDtsISN entity=(IOFormDtsISN)p_BE;				
				IOFormDtsISNCtl control=new IOFormDtsISNCtl(sqlTrans);
				entity.ID=(int)EntityIDTable.GetID((long)SysEntity.WH_IOFormDtsISN,sqlTrans);
