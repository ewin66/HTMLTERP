...
{
	/// <summary>
	/// 目的：Data_VDsignerFile实体业务规则类
	/// 作者:陈加海
	/// 创建日期:2012-4-19
	/// </summary>
	public class VDsignerFileRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public VDsignerFileRule()
		{
		}
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM Data_VDsignerFile WHERE 1=1";
                sql += p_condition;
        ...
        #region 新方法
        public void RSave(BaseEntity[] p_BE,int p_ID)
        {
            try
            {
                IDBTransAccess sqlTrans = TransSysUtils.GetDBTransAccess();
        ...
        public void RSave(BaseEntity[] p_BE,int p_ID, IDBTransAccess sqlTrans)
        {
            try
            {
                string sql = "DELETE Data_VDsignerFile WHERE MainID="+SysString.ToDBString(p_ID);
                sqlTrans.ExecuteNonQuery(sql);
                for (int i = 0; i < p_BE.Length; i++)
                {
                    VDsignerFileRule rule = new VDsignerFileRule();
                    VDsignerFile entity = (VDsignerFile)p_BE[i];
                    entity.MainID = p_ID;
                    entity.Seq = i + 1;
