...
{
	/// <summary>
	/// 目的：Data_ItemCheckStandardPhy实体业务规则类
	/// 作者:陈加海
	/// 创建日期:2014/6/20
	/// </summary>
	public class ItemCheckStandardPhyRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public ItemCheckStandardPhyRule()
		{
		}
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM Data_ItemCheckStandardPhy WHERE 1=1";
                sql += p_condition;
        ...
        public void RSave(Item p_Entity, BaseEntity[] p_BE, IDBTransAccess sqlTrans)
        {
            try
            {
                string sql = "DELETE FROM Data_ItemCheckStandardPhy WHERE MainID=" + p_Entity.ID.ToString();
                sql += " AND ID NOT IN" + string.Format("({0})", GetIDExist(p_BE));
                sqlTrans.ExecuteNonQuery(sql);//删除原单据里应该删除的明细数据，即数据库里有但是UI里已经删除的数据
                for (int i = 0; i < p_BE.Length; i++)
                {
                    ItemCheckStandardPhy entitydts = (ItemCheckStandardPhy)p_BE[i];
