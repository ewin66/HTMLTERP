...
{
	/// <summary>
	/// 目的：Data_VendorDes实体业务规则类
	/// 作者:章文强
	/// 创建日期:2012-8-13
	/// </summary>
	public class VendorDesRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public VendorDesRule()
		{
		}
		...
		{
			VendorDes entity=(VendorDes)p_BE;
		}	
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            VendorDes entity = (VendorDes)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, VendorDes.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Data_VendorDes WHERE 1=1";
                sql += p_condition;
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				VendorDes entity=(VendorDes)p_BE;				
				VendorDesCtl control=new VendorDesCtl(sqlTrans);
				entity.ID=(int)EntityIDTable.GetID((long)SysEntity.Data_VendorDes,sqlTrans);
        ...
        public void RAdd(BaseEntity p_BE,BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                for (int i = 0; i < p_BE2.Length; i++)
                {
                    ...
                    VendorDesDts entityDts = (VendorDesDts)p_BE2[i];
        ...
        public void RUpdate(BaseEntity p_BE,BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                VendorDesCtl control = new VendorDesCtl(sqlTrans);
                control.Update(entity);
                string sql = "DELETE Data_VendorDesDts WHERE MainID="+SysString.ToDBString(entity.ID);
