...
{
	/// <summary>
	/// 目的：Dev_ColorCard实体业务规则类
	/// 作者:qiuchao
	/// 创建日期:2014/6/27
	/// </summary>
	public class ColorCardRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public ColorCardRule()
		{
		}
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            ColorCard entity = (ColorCard)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, ColorCard.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM Dev_ColorCard WHERE 1=1";
                sql += p_condition;
        ...
        public DataTable RShowDts(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Dev_ColorCardDts WHERE 1=1";
                sql += p_condition;
                return SysUtils.Fill(sql);
            }
            catch (BaseException)
        ...
        public void RAdd(BaseEntity p_BE, BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                ColorCardDtsRule ruledts = new ColorCardDtsRule();
                ruledts.RSave((ColorCard)p_BE, p_BE2, sqlTrans);//保存从表
		...
		public void RDelete(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
                ...
                string sql = "DELETE FROM Dev_ColorCardDts WHERE MainID=" + entity.ID.ToString();
