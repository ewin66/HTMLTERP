...
{
	/// <summary>
	/// 目的：Dev_GBJCDts实体业务规则类
	/// 作者:刘德苏
	/// 创建日期:2012/4/28
	/// </summary>
	public class GBJCLRDtsRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public GBJCLRDtsRule()
		{
		}
		...
		{
			GBJCLRDts entity=(GBJCLRDts)p_BE;
		}	
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            GBJCLRDts entity = (GBJCLRDts)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, GBJCLRDts.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        /// </summary>
        /// <param name="p_condition"></param>
        public DataTable RLShow(string p_condition)
        {
            try
            {
                return RShow(p_condition, "*");
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				GBJCLRDts entity=(GBJCLRDts)p_BE;				
				GBJCLRDtsCtl control=new GBJCLRDtsCtl(sqlTrans);
                //entity.ID=(int)EntityIDTable.GetID((long)SysEntity.Dev_GBJCDts,sqlTrans);
                entity.ID = (int)EntityIDTable.GetID((long)SysEntity.Dev_GBJCLRDts, sqlTrans);
