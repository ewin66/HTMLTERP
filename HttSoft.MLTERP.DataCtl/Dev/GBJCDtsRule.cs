...
namespace HttSoft.MLTERP.DataCtl
{
	...
	public class GBJCDtsRule
	{
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            GBJCDts entity = (GBJCDts)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, GBJCDts.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RLShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Dev_GBJCDts WHERE 1=1 AND GBStatusID=2";
                sql += p_condition;
                return SysUtils.Fill(sql);
            }
            catch (BaseException)
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				GBJCDts entity=(GBJCDts)p_BE;				
				GBJCDtsCtl control=new GBJCDtsCtl(sqlTrans);
