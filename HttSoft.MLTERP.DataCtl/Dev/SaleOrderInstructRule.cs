...
{
	/// <summary>
	/// 目的：Sale_SaleOrderInstruct实体业务规则类
	/// 作者:tanghao
	/// 创建日期:2015/5/22
	/// </summary>
	public class SaleOrderInstructRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public SaleOrderInstructRule()
		{
		}
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            SaleOrderInstruct entity = (SaleOrderInstruct)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, SaleOrderInstruct.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM Sale_SaleOrderInstruct WHERE 1=1";
                sql += p_condition;
        ...
        public void RAdd(BaseEntity p_BE, BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                SaleOrderInstructDtsRule ruledts = new SaleOrderInstructDtsRule();
                ruledts.RSave((SaleOrderInstruct)p_BE, p_BE2, sqlTrans);//保存从表
