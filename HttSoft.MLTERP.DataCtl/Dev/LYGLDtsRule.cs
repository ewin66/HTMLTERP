...
{
	/// <summary>
	/// 目的：Dev_LYGLDts实体业务规则类
	/// 作者:陈加海
	/// 创建日期:2012-4-17
	/// </summary>
	public class LYGLDtsRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Dev_LYGLDts WHERE 1=1";
                sql += p_condition;
        ...
        public void RSave(LYGL p_Entity, BaseEntity[] p_BE, IDBTransAccess sqlTrans)
        {
            try
            {
                string sql = "DELETE FROM Dev_LYGLDts WHERE MainID=" + p_Entity.ID.ToString();
                sql += " AND ID NOT IN" + string.Format("({0})", GetIDExist(p_BE));
                sqlTrans.ExecuteNonQuery(sql);//删除原单据里应该删除的明细数据，即数据库里有但是UI里已经删除的数据
                for (int i = 0; i < p_BE.Length; i++)
                {
                    LYGLDts entitydts = (LYGLDts)p_BE[i];
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				LYGLDts entity=(LYGLDts)p_BE;				
				LYGLDtsCtl control=new LYGLDtsCtl(sqlTrans);
				entity.ID=(int)EntityIDTable.GetID((long)SysEntity.Dev_LYGLDts,sqlTrans);
