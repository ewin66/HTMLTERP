...
{
	/// <summary>
	/// 目的：Buy_ItemBuyFollow实体业务规则类
	/// 作者:刘德苏
	/// 创建日期:2012/5/3
	/// </summary>
	public class ItemBuyFollowRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public ItemBuyFollowRule()
		{
		}
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            ItemBuyFollow entity = (ItemBuyFollow)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, ItemBuyFollow.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Buy_ItemBuyFollow WHERE 1=1";
                sql += p_condition;
        ...
        public DataTable RShowDts(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Buy_ItemBuyFollowDts WHERE 1=1";
                sql += p_condition;
                return SysUtils.Fill(sql);
            }
            catch (BaseException)
        ...
        public void RAdd(BaseEntity p_BE, BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                ItemBuyFollowDtsRule ruledts = new ItemBuyFollowDtsRule();
                ruledts.RSave((ItemBuyFollow)p_BE, p_BE2, sqlTrans);//保存从表
		...
		public void RDelete(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
                ...
                string sql = "DELETE FROM Buy_ItemBuyFollowDts WHERE MainID=" + entity.ID.ToString();
