...
{
	/// <summary>
	/// 目的：SMS_SParamset实体业务规则类
	/// 作者:章文强
	/// 创建日期:2012/7/11
	/// </summary>
	public class SParamsetRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public SParamsetRule()
		{
		}
		...
		{
			SParamset entity=(SParamset)p_BE;
		}	
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            SParamset entity = (SParamset)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, SParamset.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM SMS_SParamset WHERE 1=1";
                sql += p_condition;
        ...
        public string RShowStrByID(int p_ID)
        {
            try
            {
                ...
                sql = " SELECT SetValueStr FROM SMS_SParamset  WHERE  ID = " + SysString.ToDBString(p_ID);
        ...
        public int RShowIntByID(int p_ID)
        {
            try
            {
                int outstr = -100;
                string sql = string.Empty;
                sql = " SELECT SetValueInt FROM SMS_SParamset  WHERE  ID = " + SysString.ToDBString(p_ID);
                DataTable dt = SysUtils.Fill(sql);
                if (dt.Rows.Count != 0)
                {
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				SParamset entity=(SParamset)p_BE;				
				SParamsetCtl control=new SParamsetCtl(sqlTrans);
				entity.ID=(int)EntityIDTable.GetID((long)SysEntity.SMS_SParamset,sqlTrans);
