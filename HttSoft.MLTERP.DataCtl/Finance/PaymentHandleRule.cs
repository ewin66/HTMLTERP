...
{
	/// <summary>
	/// 目的：Finance_PaymentHandle实体业务规则类
	/// 作者:qiuchao
	/// 创建日期:2015/6/16
	/// </summary>
	public class PaymentHandleRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public PaymentHandleRule()
		{
		}
		...
		{
			PaymentHandle entity=(PaymentHandle)p_BE;
		}	
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            PaymentHandle entity = (PaymentHandle)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, PaymentHandle.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Finance_PaymentHandle WHERE 1=1";
                sql += p_condition;
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				PaymentHandle entity=(PaymentHandle)p_BE;				
				PaymentHandleCtl control=new PaymentHandleCtl(sqlTrans);
				entity.ID=(int)EntityIDTable.GetID((long)SysEntity.Finance_PaymentHandle,sqlTrans);
        ...
        /// </summary>
        /// <param name="p_BE">要新增的实体</param>
        public void RSave(string p_OrderFormNo, BaseEntity[] p_BE)
        {
            try
            {
                IDBTransAccess sqlTrans = TransSysUtils.GetDBTransAccess();
                try
                {
                    ...
                    this.RSave(p_OrderFormNo, p_BE, sqlTrans);
