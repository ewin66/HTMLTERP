...
{
	/// <summary>
	/// 目的：Finance_RecPayDts实体业务规则类
	/// 作者:周富春
	/// 创建日期:2015/3/25
	/// </summary>
	public class RecPayDtsRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public RecPayDtsRule()
		{
		}
		...
		{
			RecPayDts entity=(RecPayDts)p_BE;
		}	
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            RecPayDts entity = (RecPayDts)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, RecPayDts.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM Finance_RecPayDts WHERE 1=1";
                sql += p_condition;
		...
		public void RAdd(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
				...
				RecPayDts entity=(RecPayDts)p_BE;				
				RecPayDtsCtl control=new RecPayDtsCtl(sqlTrans);
				entity.ID=(int)EntityIDTable.GetID((long)SysEntity.Finance_RecPayDts,sqlTrans);
