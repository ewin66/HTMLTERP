...
{
	/// <summary>
	/// 目的：Finance_CostRegister实体业务规则类
	/// 作者:杨光
	/// 创建日期:10/21/2011
	/// </summary>
	public class CostRegisterRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
		public CostRegisterRule()
		{
		}
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            CostRegister entity = (CostRegister)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, CostRegister.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM Finance_CostRegister WHERE 1=1";
                sql += p_condition;
        ...
        public void RAdd(BaseEntity p_BE, BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                CostRegisterDtsRule ruledts = new CostRegisterDtsRule();
                ruledts.RSave((CostRegister)p_BE, p_BE2, sqlTrans);//保存从表
		...
		public void RDelete(BaseEntity p_BE,IDBTransAccess sqlTrans)
		{
			try
			{
                ...
                string sql = "DELETE FROM Finance_CostRegisterDts WHERE MainID=" + entity.ID.ToString();
