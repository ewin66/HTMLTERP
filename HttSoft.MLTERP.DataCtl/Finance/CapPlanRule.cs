...
{
	/// <summary>
	/// 目的：Finance_CapPlan实体业务规则类
	/// 作者:章文强
	/// 创建日期:2012/6/11
	/// </summary>
	public class CapPlanRule
	{
	    /// <summary>
        /// 构造函数
        /// </summary>
        ...
        private bool CheckFieldValueIsExist(BaseEntity p_BE, string p_FieldName, string p_FieldValue, IDBTransAccess p_sqlTrans)
        {
            CapPlan entity = (CapPlan)p_BE;
            bool ret = false;
            string sql = string.Format(" SELECT {0} FROM {1} WHERE 1=1 AND {0}={2} AND {3}<>{4}", p_FieldName, CapPlan.TableName, SysString.ToDBString(p_FieldValue), "ID", entity.ID);
        ...
        public DataTable RShow(string p_condition, string p_FieldName)
        {
            try
            {
                ...
                string sql = "SELECT " + p_FieldName + " FROM UV1_Finance_CapPlanDts WHERE 1=1";
                sql += p_condition;
        ...
        public void RUpdate(BaseEntity p_BE,BaseEntity[] p_BE2, IDBTransAccess sqlTrans)
        {
            try
            {
                ...
                string sql = "DELETE  Finance_CapPlanDts WHERE MainID="+SysString.ToDBString(entity.ID);
